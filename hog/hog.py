...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    i, single, total, is_one = 0, 0, 0, False
    while i < num_rolls:
        single = dice()
        if single == 1:
            is_one = True
...
def perfect_square(score):
    sqrt_score = sqrt(score)
    return (sqrt_score - int(sqrt_score) == 0)
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while i <= 100:
        ...
        j = 0
        if i <= 99:
            if strategy(i,j) != strategy(i+1,j):
                return False
        i +=1
    return True
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    rolls, index, avg = 1, 0, 0
    while rolls <= 10:
        value = make_averaged(roll_dice, total_samples)(rolls, dice)
        if avg < value:
            avg, index = value, rolls
        rolls += 1
    return index
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    square_score = square_update(0, score, opponent_score) - score
    if tail_score >= threshold or tail_score != square_score:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    if current score is above 90, return safer bet; roll smaller number of dice
    compare 
    """
    # BEGIN PROBLEM 12
    if score >= 90:
