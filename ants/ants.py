...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, lowerbound = lower_bound, upperbound = upper_bound):
        ...
        while not current.is_hive:
            bees = random_bee(current.bees)
            if bees and self.lower_bound <= range <= self.upper_bound:
                return bees
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # Ant dies: damage -> amount + self.damage
        if amount >= self.health:
            damage += self.damage
            for bee in bees:
                Insect.reduce_health(bee, damage)
            super().reduce_health(amount)
        
        # Ant does not die: damage -> amount
        else:
            for bee in bees:
                Insect.reduce_health(bee, damage)
            super().reduce_health(amount)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        if not isinstance(other, ContainerAnt) and not self.ant_contained:
            return True
        return False
        # END Problem 8a
    ...
    def action(self, gamestate):
        # BEGIN Problem 8a
        contained = self.ant_contained
        if contained:
            contained.action(gamestate)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        # double the damage of previous ants
        previous_place = self.place.exit
        while previous_place:
            if previous_place.ant:
                if not previous_place.ant.marker:
                    previous_place.ant.double()
                    previous_place.ant.marker = True
                if previous_place.ant.is_container and previous_place.ant.ant_contained and not previous_place.ant.ant_contained.marker:
                    previous_place.ant.ant_contained.double()
                    previous_place.ant.ant_contained.marker = True
            previous_place = previous_place.exit        
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        # BEGIN Problem EC
        if target in self.bees:
           self.bees[target] += 3
        else:
            self.bees[target] = 2

        def action_function(gamestate):
            if target in self.bees and self.bees[target] > 0:     
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not current.is_hive:
            ...
            ant = current.ant
            if ant and ant.is_container and ant.ant_contained and ant.ant_contained is not self:
                insects[ant.ant_contained] = distance
            elif ant and ant is not self:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage = self.damage - (0.25 * distance + self.insects_shot * 0.0625)
        if damage <= 0:
            return 0
        return damage 
        # END Problem Optional 2
