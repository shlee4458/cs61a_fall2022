...
def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return [[item] + element for element in nested_list]
...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        lst = insert_into_all(s[0], subseqs(s[1:]))
        return lst + subseqs(s[1:])
...
class VendingMachine:
    ...
    def vend(self):
        ...
        if change < 0:
            return f"Please update your balance with ${-change} more funds."
        self.balance = 0
        self.stock -= 1
...
def trade(first, second):
    ...
    equal_prefix = lambda m, n: sum(first[:m]) == sum(second[:n]) 
    while not equal_prefix(m, n) and m <= len(first) and n <= len(second):
...
def reverse_other(t):
    ...
    def helper(t, reverse):   
        ...
        for i in range(len(t.branches)):
            single_branch = t.branches[i]
            if reverse:
                single_branch.label = labels[i]
            helper(single_branch, not reverse)
    helper(t,True)
