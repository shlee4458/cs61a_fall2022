...
def balanced(m):
    ...
    True
    """
    if end(left(m)) == 'planet' or end(right(m)) == 'planet':
        return True
    else:
        if length(left(m)) * total_weight(end(left(m))) != length(right(m)) * total_weight(end(right(m))):
            return False
        else:
            return balanced(end(left(m))) and balanced(end(left(m)))
...
def has_path(t, word):
    ...
    if is_leaf(t):
        ...
        return word in tree(label(t), [has_path(b, word) for b in branches(b)])
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert upper_bound(y) * lower_bound(y) > 0, "interval contains 0."
