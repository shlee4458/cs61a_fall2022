...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    filtered_list = [paragraph for paragraph in paragraphs if select(paragraph)]
...
def about(topic):
    ...
    def select(paragraph):
        words_list = [remove_punctuation(word) for word in paragraph.lower().split()]
        for word in topic:
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    d = {word:diff_function(typed_word, word, limit) for word in word_list}
...
def feline_fixes(typed, source, limit):
    ...
    # BEGIN PROBLEM 6
    # if length difference add length difference to the required substitution
    ...
    incorrect = typed[0] != source[0]

    incorrect = incorrect + feline_fixes(typed[1:], source[1:], limit - incorrect)
...
def minimum_mewtations(start, goal, limit):
    ...
    elif start in goal or goal in start:
        return abs(len(goal) - len(start))
    
    else:
        mew = start[0] != goal[0]
        add = mew + minimum_mewtations(start, goal[1:], limit - 1)
        remove = mew + minimum_mewtations(start[1:], goal, limit - 1)
        substitute = mew + minimum_mewtations(start[1:], goal[1:], limit - 1)
...
def time_per_word(words, times_per_player):
    ...
    for p in range(len(times_per_player)):
        for t in range(len(words)):
            times_single.append(times_per_player[p][t+1]-times_per_player[p][t])
        times.append(times_single)
        times_single = []
...
def fastest_words(match):
    ...
    for w in word_indices:
        for p in player_indices:
            if time(match, p, w) < fastest_time:
                fastest_player = p
                fastest_time = time(match, p, w)
        fastest_words_list[fastest_player].append(get_word(match, w))
        fastest_player = 0
        fastest_time = float('inf')
