...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        """
        if len(player.hand) > 0:
            card_power = [card.power(opponent_card) for card in player.hand]
            max_card = max(card_power)
            max_index = card_power.index(max_card)
            self.attack = self.attack + player.hand[max_index].attack
            self.defense = self.defense + player.hand[max_index].defense
            best_card = player.hand.pop(max_index)
        else:
            best_card = False
        # You should add your implementation above this.
        if best_card:
